...
class Bertrand:
	...
	def draw_triangle(self, OneEndPoint, center=(0,0)):
		...
		xA = Tricenterx + (x-Tricenterx)*np.cos(angle)-(y-Tricentery)*np.sin(angle)
		yA = Tricentery + (x-Tricenterx)*np.sin(angle)+(y-Tricentery)*np.cos(angle)
		A = (xA, yA)
		points = np.vstack([points, [xA, yA]])
		self.ax.plot(xA, yA, color='cyan', marker='*')

		angle = np.deg2rad(-120)
		xB = Tricenterx + (x-Tricenterx)*np.cos(angle)-(y-Tricentery)*np.sin(angle)
		yB = Tricentery + (x-Tricenterx)*np.sin(angle)+(y-Tricentery)*np.cos(angle)
		B = (xB, yB)
	...
	def first_method(self):
		...
		for i in range(n):
			...
			self.draw_triangle(P, center=(0,0)) 
			plt.savefig('C:/Users/12102/Desktop/Data/{}.png'.format(i))
			self.ax.plot([M1[0], M2[0]], [M1[1], M2[1]], color='gray')  
			
		print('The probability for the length of chord is greater than sqrt(3)*r is: {}/{} = {}'.format(fav, n, fav/n))
		...
		self.fig.suptitle('Bertrand paradox (random two points)')
		self.draw_circle(r, center=(0,0))
		self.ax.scatter(center_points[:,0], center_points[:,1], color='orange', marker='.')
		self.draw_triangle(P, center=(0,0))
...
def main():
	...
	print('\t3 (random middle point along the radius)') ### chord angle is determined by the middle point
...
if __name__ == '__main__':
	...
	'''
	print('Delete files...')
	for f in glob.glob(f"C:/Users/12102/Desktop/Data/*"):
		os.remove(f)
	...
	images = []
	filenames = [image for image in glob.glob(f"C:/Users/12102/Desktop/Data/*.png")]
	filenames = sorted(filenames, key=len)
	print(filenames[0:15])
	
	for filename in filenames:
		images.append(imageio.imread(filename))
	imageio.mimsave('Method{}.gif'.format(choice), images) 